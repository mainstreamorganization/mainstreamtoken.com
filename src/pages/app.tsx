...
function App() {
    ...
    const generateLicenseString = () => {
        let selectedTypes = Object.keys(licenseTypes).filter(key => licenseTypes[key]).join('');
        let license = `MTLv0.1-${selectedTypes}/${tokenAmount}`;

        if (fee && splits.length > 0) {
            license += `-${fee}`;
    ...
    return (
        <Layout title="MTL License Generator" description="Generate MTL License Strings">
            <div className={styles.appContainer}>
                <h1>MTL License Generator</h1>
